...
CREATE PROC SP_DA_BAN_MOI_CN_THEO_QUY @QUY INT, @NAM INT --SẢN PHẨM ĐÃ BÁN THEO MỖI CHI NHÁNH THEO QUÝ
AS
...
CREATE PROC SP_DA_BAN_MOI_CN_THEO_THANG @THANG INT, @NAM INT --THỐNG KÊ SẢN PHẨM ĐÃ BÁN THEO THÁNG
AS
	SELECT GH.ID_SP, SP.TEN_SP, SUM(SO_LUONG)
	FROM GIO_HANG GH, DON_HANG DH, SAN_PHAM SP
	WHERE SP.ID_SP=GH.ID_SP AND GH.ID_DH=DH.ID_DH AND YEAR(DH.NGAY_BAN)=@NAM 
...
CREATE PROC SP_DA_BAN_MOI_CN_THEO_NAM @NAM INT -- THỐNG KÊ SẢN PHẨM ĐÃ BÁN THEO NĂM
AS
	SELECT GH.ID_SP, SP.TEN_SP, SUM(SO_LUONG)
	FROM GIO_HANG GH, DON_HANG DH, SAN_PHAM SP
	WHERE SP.ID_SP=GH.ID_SP AND GH.ID_DH=DH.ID_DH AND YEAR(DH.NGAY_BAN)=@NAM 
